...
namespace VirtoCommerce.ImportModule.Data.Importers.ShopifyProductImporter
{
    public class ShopifyProductLine
    {
        ...
        {
            var result = ExType<ProductDetails>.New();
        ...
        public OfferDetails ToOfferDetails()
        {
            var result = ExType<OfferDetails>.New();

            result.OuterId = Handle;
            result.Name = Title;
            result.Prices = new List<OfferPrice>();
            if (decimal.TryParse(VariantPrice, out decimal listPrice))
            {
                var offerPrice = ExType<OfferPrice>.New();
                offerPrice.ListPrice = listPrice;
                result.Prices.Add(offerPrice);
            }
